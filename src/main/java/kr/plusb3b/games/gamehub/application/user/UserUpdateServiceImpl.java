...
public class UserUpdateServiceImpl implements UserUpdateService {
    ...
    public void updateUserProfile(Long mbId, RequestUserUpdateDto userUpdateDto) {
        ...
        // 변경 전 값들 로깅
        System.out.println("[DEBUG] 변경 전 닉네임: " + user.getMbNickname());
        System.out.println("[DEBUG] 변경 전 프로필URL: " + user.getMbProfileUrl());
        System.out.println("[DEBUG] 변경 전 상태메시지: " + user.getMbStatusMessage());
        System.out.println("[DEBUG] 변경 전 이메일: " + userPrivate.getPriEmail());

        // 변경할 값들 로깅
        System.out.println("[DEBUG] 변경할 닉네임: " + userUpdateDto.getMbNickName());
        System.out.println("[DEBUG] 변경할 프로필URL: " + userUpdateDto.getMbProfileUrl());
        System.out.println("[DEBUG] 변경할 상태메시지: " + userUpdateDto.getMbStatusMessage());
        System.out.println("[DEBUG] 변경할 이메일: " + userUpdateDto.getPriEmail());
        ...
        // 변경 후 값들 확인
        System.out.println("[DEBUG] 변경 후 닉네임: " + user.getMbNickname());
        System.out.println("[DEBUG] 변경 후 프로필URL: " + user.getMbProfileUrl());
        System.out.println("[DEBUG] 변경 후 상태메시지: " + user.getMbStatusMessage());
        System.out.println("[DEBUG] 변경 후 이메일: " + userPrivate.getPriEmail());

        // 명시적 저장 (변경감지가 안될 경우 대비)
        userRepository.save(user);
        userPrivateRepo.save(userPrivate);

        System.out.println("[DEBUG] 명시적 저장 완료");
        System.out.println("[DEBUG] =========================");
    }
    @Override
    @Transactional
    public void updateUserPassword(Long mbId, String currentPassword, String newPassword) {
        ...
        // 현재 비밀번호 확인 로직 수정
        if(!passwordEncoder.matches(currentPassword, userAuth.getAuthPassword())){
